<div class="grid grid-cols-1 gap-6 max-w-3xl">
    ...
    <div>
        ...
        <div class="space-y-2">
            <div class="flex items-center">
                <input id="printer_browser" name="printer.type" type="radio" value="browser" {{ old('printer.type', $settings['printer.type'] ?? 'browser') == 'browser' ? 'checked' : '' }}
            ...
            <div class="flex items-center">
                <input id="printer_escpos" name="printer.type" type="radio" value="escpos" {{ old('printer.type', $settings['printer.type'] ?? 'browser') == 'escpos' ? 'checked' : '' }}
                    class="focus:ring-slate-500 h-4 w-4 text-slate-600 border-gray-300">
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Lebar Kertas Thermal (mm)</label>
        <select name="printer.paper_width"
            ...
            <option value="58" {{ old('printer.paper_width', $settings['printer.paper_width'] ?? '80') == '58' ? 'selected' : '' }}>58mm</option>
            <option value="80" {{ old('printer.paper_width', $settings['printer.paper_width'] ?? '80') == '80' ? 'selected' : '' }}>80mm</option>
        </select>
    </div>
