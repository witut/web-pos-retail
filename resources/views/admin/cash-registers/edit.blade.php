<x-layouts.admin :title="'Edit Mesin Kasir'">
    <div class="mb-6 flex items-center">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Mesin Kasir</h2>
            <p class="text-sm text-gray-500">Ubah data terminal POS {{ $cashRegister->name }}.</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
        <form action="{{ route('admin.cash-registers.update', $cashRegister) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $cashRegister->name) }}" required
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        class="block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500 sm:text-sm">
                        <option value="active" {{ old('status', $cashRegister->status) == 'active' ? 'selected' : '' }}>
                            Aktif - Bisa digunakan untuk buka sesi</option>
                        <option value="inactive" {{ old('status', $cashRegister->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif (Inactive)</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="ip_address" id="ip_address"
                        value="{{ old('ip_address', $cashRegister->ip_address) }}"
                        class="block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500 sm:text-sm"
